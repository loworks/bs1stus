  ...
  useEffect(() => {
    if (state.transitionState === "exsitStart") {
      ...
      if (isTouch && !isCategory) {
        //Transition.JournalPostTransition.killColorAnimation();
      }
    }
  ...
  useEffect(() => {
    ...
    if (!cont.classList.contains("init-loaded")) {
      Transition.InitTransition.Start().call(function () {
        ...
        if (isCategory) {
          //Transition.AboutTransition.execute();
        } else {
          Transition.JournalPostTransition.execute();
        }
      });
    ...
    return () => {
      if (isCategory) {
        // Transition.AboutTransition.cleanup();
      } else {
        Transition.JournalPostTransition.cleanup();
      }
